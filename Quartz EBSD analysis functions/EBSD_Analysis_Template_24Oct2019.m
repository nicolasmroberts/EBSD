...
figure,
plotIPDF(ebsd('Quartz-new').orientations, [vector3d.X, vector3d.Y, vector3d.Z], 'antipodal', 'contourf')
mtexColorbar
...
plot(grains.boundary,'linewidth',1, 'linecolor','white')
hold off
...
plot(grains)
hold on
plot(grains.boundary,'linewidth',0.25,'linecolor','white')
hold off
%% SPO 

[allOmega, a, b] = computeSPO(grains)

plotSPORose(allOmega, 10)
...
[grainsSmall, ebsdSmall] = constructGrains(ebsd,1); %1 degree threshhold for grains

%% Plot small grains

figure, 
%plot(ebsdSmall('Quartz'), ebsdSmall('Quartz').orientations,'coordinates','on')
plot(grains)
hold on
plot(grainsSmall.boundary,'linewidth',1, 'linecolor','white')
hold off


%% COMPUTE THE UNCORRELATED MISORIENTATIONS OF QUARTZ

[uncorrMis, odf, uncorrMDF, Mindex] = computeUncorrelatedMiso(ebsdSmall,'Quartz-new');
...
figure,
plotAxisDistribution(uncorrMis,'smooth', 'antipodal','contourf')
mtexColorMap(flipud(viridis))
mtexColorbar
mtexTitle('uncorrelated axis distribution')
    
%% Untextured axis distribution
figure,
plotAxisDistribution(ebsd('Quartz-new').CS,ebsd('Quartz-new').CS,'antipodal','contourf')
...
figure,
plotAxisDistribution(boundaryMisoAxes(boundaryMisoAxes.angle./degree < 10),'contourf')
...
sampleName = 'AME18_100XZ'; % Compatible with both .ctf and .cpr/crc formats

[grainSize, OneStanDev, grainsMergedReX, grainsReX] = recrystallizedGrainSize(ebsd, grains,CS, sampleName);

%%

grainsRel = grains(~ismember(grains.id, grainsReX.id))
grainsRel_limited = grainsRel(grainsRel.grainSize > 0)

grainsReX_limited = grainsReX(grainsReX.grainSize > 0)

grainsQuartzReX = grainsReX_limited('Quartz-new');


onePointPerGrainQuartzPF(grainsReX_limited('Quartz-new'))
